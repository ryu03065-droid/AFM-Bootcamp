...
function publicUser(row) {
  return { id: row.id, username: row.username, nickname: row.nickname, bio: row.bio };
}
...
app.put('/api/profile', requireAuth, async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      'UPDATE community_app_users SET nickname = $1, bio = $2 WHERE id = $3 RETURNING id, username, nickname, bio',
